...
export const executeSetValueOperation = (child: Node, context: ExecutionContext): void => {
  ...
  const { listName, index, valueExpression } = parseSetValueData(child.data, lists);
...
export const processSetValueOperation = (
  setNode: Node,
  lists: ListMap,
  state: StateMap,
  loopVariables?: LoopVariablesMap,
): ListMap => {
  const { listName, index, valueExpression } = parseSetValueData(setNode.data, lists);
